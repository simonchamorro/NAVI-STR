...
def train(opt):
    ...
    film_gen = FiLMGen(input_dim=200, emb_dim=1000, cond_feat_size=18944, init_xavier=opt.init_xavier)
    if torch.cuda.is_available():
        film_gen.cuda()
    ...
    while(True):
        ...
        # Get 5 random number on the same street as the selected number
        num_hn = 5
        ...
        # Only house number with 4 digits can be converted.
