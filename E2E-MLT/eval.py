...
def evaluate_image(img, detections, gt_rect, gt_txts, iou_th=0.5, iou_th_vis=0.5, iou_th_eval=0.5, eval_text_length = 1):
...
if __name__ == '__main__': 
  ...
  parser.add_argument('-eval_text_length', type=int, default=1)
  parser.add_argument('-gt_bbox', type=int, default=3)
  ...
  with torch.no_grad():
    ...
    for tp in sorted(nums, key=lambda images: images[0]):
      ...
      detections = np.expand_dims(np.append(gt_rect, 10000.), axis=0) # only if we're spoofing TODO: add flag
      ...
      if args.evaluate == 1:
        ...
        tp, tp_e2e, gt_e2e, tp_e2e_ed1, detection_to_gt  = evaluate_image(pix, detections_out, gt_rect, gt_txts, eval_text_length=eval_text_length)
        tp_all += tp 
        gt_all += len(gt_txts)
        tp_e2e_all += tp_e2e
        gt_e2e_all += gt_e2e
        tp_e2e_ed1_all += tp_e2e_ed1
        detecitons_all += len(detections_out)
      ...
      det_no = 0
      for box, det_text in detections_out:
